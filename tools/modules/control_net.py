...
#TIP: change orign UNet to can add control
@MODEL.register_class()
class ControlledUnet_T2VBaseModel(UNetSD_T2VBase):
    def forward(self, 
        x,
        t,
        x0=None,
...
@MODEL.register_class()
class ControlNet_T2VBase(nn.Module):
    def __init__(self,
            config=None,
            in_dim=4,
            dim=512,
            ...
            **kwargs):
        super(ControlNet_T2VBase, self).__init__()
        ...
        #TIP: init condition
        #map: dims=2, hint_channels=self.in_dim, model_channels=dim
        self.input_hint_block = TimestepEmbedSequential(
            conv_nd(2, self.in_dim, 16, 3, padding=1),
            nn.SiLU(),
            ...
            nn.SiLU(),
            zero_module(conv_nd(2, 256, dim, 3, padding=1))
        )
    ...
    def forward(self, 
        ...
        **kwargs):
        ...
        single_hint = self.input_hint_block(hint, embeddings, context)
        guided_hint = torch.zeros(batch, c, f, h, w, 
                          dtype=single_hint.dtype, device=single_hint.device)
        guided_hint[:, :, 0, :, :] = single_hint

        assert torch.equal(guided_hint[:, :, 0, :, :], single_hint), "the first frame is not equal"
        assert guided_hint[:, :, 1:, :, :].sum() == 0, "other frames are not zero"

        guided_hint = rearrange(guided_hint, 'b c f h w -> (b f) c h w')
        x = rearrange(x, 'b c f h w -> (b f) c h w')
        outs = []
